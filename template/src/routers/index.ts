 ...
 * @param meta.isHide ==> Whether to hide in the menu (usually detail pages need to be hidden)
...
router.beforeEach(async (to, from, next) => {
  ...
  // 3. Check if accessing login page, if has Token stay on current page, if not reset router to login page
  ...
  // 4. Check if the accessed page is in the route whitelist (static routes), if so allow directly
  if (ROUTER_WHITE_LIST.includes(to.path)) return next();

  // 5. Check if has Token, if not redirect to login page
